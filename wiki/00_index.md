...
- **39 engines**, all with a page.
- **73 notebooks**, all executing clean (393/393 code cells, verified 2026-07-28;
  `core/19_bao_mass_gap` added 2026-07-30, 18/18 cells verified separately;
  `engines/14_archimedes_screw` added 2026-08-04, not yet executed end to end).
...
| [d_star_rg.md](d_star_rg.md) | `engine/d_star_rg.py` (deferred) | THEORETICAL | The CD doubling IS the Wilson RG step; fixed point d*_RG is dimensionally **8** (persistent octonion), numerically 0.24631, closed form OPEN. Flow is a σ=½ saddle (λ≈6) → approach on the stable manifold. **Renormalization is the machine, not a move** — the void is projected (exact resummation), never subtracted. Windows of order ARE renormalization (Feigenbaum). Full spec: `FourthAgePapers/DStarRG/` |
...
| Page | Engine | Floor | Key Result |
|------|--------|-------|-----------|
| [bao_mass_gap.md](bao_mass_gap.md) | `modules/bao_mass_gap/` | ESTABLISHED | **Δ=0.0007073575=1/(1000√2)** NOT 1/√2000; residue of BAO decomposition; invariant in n; 7/7 checks, 0 free params |
...
| The 7 box-kite charts have **zero cross-strut edges** — mutually disconnected under ZD adjacency. Are the transition maps the PSL(2,7) action on struts, or is there genuinely no global medium? | box_kite.md; Ainulindale wiki/84 |
...
| [generational_lineage_map.md](generational_lineage_map.md) | **ValaQuenta as the derivation machinery** — which engines feed which part of the Generational Lineage engine (the floor it rolls to · the domain it decomposes in · the ladder), which are paired (`emerger` = the ascent mirror), adjacent, or outside the jurisdiction, and how it all relates. The jurisdiction = the *structure of an operation*, not numbers/physics/spectra. | ESTABLISHED — reference map |
